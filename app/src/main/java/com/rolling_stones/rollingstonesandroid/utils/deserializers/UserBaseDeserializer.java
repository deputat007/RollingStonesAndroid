...
public class UserBaseDeserializer implements JsonDeserializer<UserBase> {
    @Override
    public UserBase deserialize(JsonElement json, Type typeOfT,
                                JsonDeserializationContext context) throws JsonParseException {
        try {
            final JsonObject user = json.getAsJsonObject();

            return new UserBase(user.get(SerializedNames.UserBase.ID).getAsInt(),
                    ...
                    user.get(SerializedNames.UserBase.IS_ONLINE).getAsBoolean());
